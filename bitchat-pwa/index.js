...
function generateMessagePacket(senderId, senderName, content, ttl = 3) {
    ...
    const messageLength = 1 + 8 + 1 + uidBytes.length + 1 + nameBytes.length + 2 + contentBytes.length + 1 + senderId.length;
    ...
    uint8View.set(new Uint8Array([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]), offset);
    offset += 8;
...
async function sendMessages() {
    ...
    const numMessages = parseInt(document.getElementById('numMessages')?.value || 1);
    ...
    try {
        ...
        for (let i = 0; i < numMessages; i++) {
            ...
            const messagePacket = generateMessagePacket(senderId, senderName, message, 5);
...
async function broadcastToAll() {
    try {
        ...
        log('Note: Browser will show device picker for each device. Select all BitChat devices.');
        ...
        while (continueBroadcast) {
            try {
                ...
                try {
                    ...
                    for (let i = 0; i < numMessages; i++) {
                        log(`Sending message ${i + 1}/${numMessages} to ${device.name || 'Device'}...`);
                        const messagePacket = generateMessagePacket(senderId, senderName, message, 5);
                    ...
                    log(`✓ Sent ${numMessages} message(s) to ${device.name || 'Device'}`, 'success');
                    
                    await server.disconnect();
                    
                } catch (error) {
                    log(`Failed to send to ${device.name || 'Device'}: ${error.message}`, 'error');
                }
        ...
        if (devicesFound > 0) {
            log(`Broadcast complete! Sent ${messagesSent} message(s) to ${devicesFound} device(s)`, 'success');
...
document.getElementById('broadcastBtn').addEventListener('click', broadcastToAll)
...
function parseReceivedPacket(data) {
    ...
    try {
        ...
        if (packetType1 === 0x01 && packetType2 === 0x01) {
            ...
            const senderName = decoder.decode(uint8View.slice(offset, offset + nameLength));
            
            return {
                type: 'announce',
                senderName,
                senderId: Array.from(senderId).map(b => b.toString(16).padStart(2, '0')).join(''),
            ...
            const decoder = new TextDecoder();
            const uid = decoder.decode(uint8View.slice(msgOffset, msgOffset + uidLength));
            msgOffset += uidLength;
            
            const nameLength = view.getUint8(msgOffset++);
            const senderName = decoder.decode(uint8View.slice(msgOffset, msgOffset + nameLength));
            msgOffset += nameLength;
            
            const contentLength = view.getUint16(msgOffset, false);
            msgOffset += 2;
            const content = decoder.decode(uint8View.slice(msgOffset, msgOffset + contentLength));
            msgOffset += contentLength;
            
            const senderIdLength = view.getUint8(msgOffset++);
            const messageSenderId = uint8View.slice(msgOffset, msgOffset + senderIdLength);
        ...
        if (!isListening) {
            ...
            if (properties.notify || properties.indicate) {
                ...
                if (properties.read) {
                    ...
                    log('Cannot receive messages - characteristic does not support read or notify', 'error');
                }
            }
        } else {
